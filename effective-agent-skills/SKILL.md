...
license: MIT
compatibility: Uses the Agent Skills standard with documented adapters for Claude Code, Codex, Kiro CLI, and OpenCode.
metadata:
...
Skills follow the open Agent Skills standard (agentskills.io), which is supported by multiple coding tools. The core folder and `SKILL.md` format are portable, but discovery paths and invocation controls are client-specific.
...
- `name` is lowercase alphanumeric with single-hyphen separators, 1–64 chars, and **exactly matches the parent folder name**
- Avoid `<` and `>` in frontmatter (they can inject into the system prompt)
- Invalid YAML silently prevents loading
- **Never put `: ` (colon + space) inside an unquoted `description`** — strict YAML parsers can reject it as a nested mapping. If the text needs a mid-sentence colon, single-quote the whole value and double any inner apostrophes: `description: 'Differentiator: finds gaps in the user''s knowledge.'`

### Portability and manual-only invocation
...
OpenCode V2 uses metadata in `SKILL.md`:
...
Kiro CLI supports automatic and slash-command activation but has no equivalent portable enforcement field. Put "manual-only; invoke explicitly" in the description so the routing contract remains clear.

For a manual-only skill shared across Claude Code, Codex, Kiro CLI, and OpenCode, include every supported configuration above. Never assume a client-specific field works elsewhere; unknown fields should remain safe to ignore.
...
- Kiro CLI: `.kiro/skills/<name>/SKILL.md` or `~/.kiro/skills/<name>/SKILL.md`.
- OpenCode: `.opencode/skills/<name>/SKILL.md`, with `.claude/skills` and `.agents/skills` also supported as compatibility locations.
