...
license: MIT
compatibility: Requires macOS 14 or newer and the cmux CLI.
metadata:
...
cmux is a native macOS terminal app for running multiple coding sessions in parallel. It exposes a CLI (`cmux`) and a Unix-socket JSON-RPC API for full topology and browser control. Prefer the CLI and resolve socket details at runtime instead of hardcoding a path.
...
When polling any cmux pane — an agent, an SSH login, or a running command — use **short `sleep` intervals (1–3s)**. Do not use `sleep 10`+ unless a slow build or similar operation genuinely needs it.
...
Browser capabilities vary by cmux version. Run `cmux browser --help` and `cmux capabilities --json` before using viewport emulation, geolocation, offline mode, tracing, network interception, or raw input.
...
Preserve both current values unless the user explicitly asks to change them. `sidebar.hideAllDetails: true` hides the entire detail block (status, branch, cwd) if needed.
...
Native session-resume and hook support varies by release. Run `cmux hooks --help` for the current integration list, including Claude Code, Codex, Kiro CLI, and OpenCode.

## Socket API (advanced)

The socket path is exposed through `CMUX_SOCKET_PATH`; current builds otherwise use `~/.local/state/cmux/cmux.sock` and may auto-discover tagged or debug sockets. Use the socket only for tight loops where subprocess spawn cost matters; otherwise prefer the CLI.

```bash
CMUX_RPC_SOCKET=${CMUX_SOCKET_PATH:-$HOME/.local/state/cmux/cmux.sock}
echo '{"jsonrpc":"2.0","id":"1","method":"workspace.list","params":{}}' | nc -U "$CMUX_RPC_SOCKET"
```

Method prefixes: `system.*`, `window.*`, `workspace.*`, `pane.*`, `surface.*`, `notification.*`, `browser.*`. Use `cmux capabilities --json` for the current method list.
