...
license: MIT
compatibility: Requires network access, jq and curl for DeepAPI, or Python 3 and yt-dlp for the fallback.
metadata:
...
- **Always name the file `Channel_Title` with spaces replaced by `_`** (e.g. `Example_Channel_title_of_video.txt`). If metadata is unavailable, fall back to the video ID.

## Primary path — DeepAPI

Read the key from the environment or the optional `~/.deepapi/env` file. Never load an entire shell profile just to obtain the key:

```bash
if [ -z "$DEEPAPI_API_KEY" ] && [ -f ~/.deepapi/env ]; then
  . ~/.deepapi/env
fi
[ -n "$DEEPAPI_API_KEY" ] || exit 2
DEEPAPI_KEY=$DEEPAPI_API_KEY
DEEPAPI_BASE=${DEEPAPI_API_BASE_URL:-https://deepapi.co}
```

Never print or log `DEEPAPI_KEY`.

Run the scrape (keep the Idempotency-Key; retries must reuse the SAME one):

```bash
TRANSCRIPT_TMP=$(mktemp -d)
IDK=$(uuidgen)
jq -n --arg url "VIDEO_URL" \
  '{url: $url, maxCostUsd: "0.05", waitForFinishSecs: 60}' \
  > "$TRANSCRIPT_TMP/request.json"
curl -fsS --max-time 120 "$DEEPAPI_BASE/v1/scrape/youtube/transcript" \
  -H "Authorization: Bearer $DEEPAPI_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: $IDK" \
  --data-binary "@$TRANSCRIPT_TMP/request.json" \
  > "$TRANSCRIPT_TMP/response.json"
```

- Non-English videos: add `"language": "de"` (etc.) to the body.
- `status: running` → wait `next.afterSecs`, then poll `next.path` with the same authorization until `succeeded` or `failed`. Stop after the service deadline; never poll indefinitely.

Extract the text and save it:

```bash
jq -r '.status' "$TRANSCRIPT_TMP/response.json"                # succeeded | running | failed
jq -r '.output[0].text' "$TRANSCRIPT_TMP/response.json" > "$OUT/$NAME.txt"
```
...
Verify that the final `.txt` exists and is non-empty. Remove only the explicit temporary directories created for this run after the output is verified.

## When to fall back to yt-dlp

- `DEEPAPI_API_KEY` unset and `~/.deepapi/env` missing.
- HTTP 402 `insufficient_credits` (tell the user that credits are unavailable, then use the fallback).
- DeepAPI request fails after one safe retry with the same idempotency key.

Tell the user whenever the fallback is used and why.

## Fallback path — yt-dlp (local)

```bash
OUT="$(pwd)"            # or the user's Downloads directory if cwd makes no sense
YTDLP_TMP=$(mktemp -d)
...
```bash
python3 - "$YTDLP_TMP" "$OUT/$NAME.txt" <<'PY'
...
- On first failure: update yt-dlp using its installation method, retry once, then stop.
...
Report the saved path; print the text if short. Do not report costs unless the user asks. If no captions exist, state that clearly and do not create an empty transcript.
